...
; layui.extend({
	setter: 'config', admin: 'admin', view: 'view'
	...
	var e = layui.setter,
		...
		l = function (a, d) {
			...
			if (b.each(function (e) {
				var i = r(this), n = i.attr('lay-id'); n === a && (l = !0, t.index = e);
			}), d = d || '新标签页', e.pageTabs)
				l || (r(s).append(['<div class="layadmin-tabsbody-item layui-show">',
					'<iframe src="' + a + '" frameborder=0" class="layadmin-iframe"></iframe>', '</div>'].join('')),
					t.index = b.length,
					i.tabAdd(o, { title: '<span>' + d + '</span>', id: a, attr: y }));
			else {
	...
	n.screen() < 2 && n.sideFlexible(),
		layui.config({ base: e.base }),
		layui.each(e.extend, function (a, i) {
			var n = {}; n[i] = '{/}' + e.base + 'lib/' + i, layui.extend(n);
		}),
		d().autoRender(),
		layui.use('common'),
