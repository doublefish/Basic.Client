layui.use(['jquery', 'layer', 'form'], function () {
	...
	var treeObj = $.fn.zTree.init($('#tree'), {
		...
		callback: {
			beforeAsync: function (treeId, treeNode) {
				...
				if (treeNode) {
					treeObj.setting.async.url = '/menu/list/' + treeNode.Id;
				} else {
					treeObj.setting.async.url = '/menu/list/' + Config.Menu.Root;
				}
				return true;
			},
			...
			onRightClick: function (event, treeId, treeNode) {
				...
				$updateStatus.on('click', { id: treeNode.Id, status: status }, function (e) {
					...
					$.ajax('/menu/updateStatus/' + node.Id, {
						type: 'put',
	...
	if (HasRight('menu/update')) {
		$('#btnSave').show();
		$('#btnReset').show();
	}
	if (HasRight('menu/updateStatus')) {
		$('#updateStatus').show();
	}
	if (HasRight('menu/delete')) {
		$('#del').show();
	}
	if (HasRight('menu/refreshcache')) {
