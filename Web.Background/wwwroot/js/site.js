...
var GetToken = function () {
	var token = null;
	var jsonString = $.cookie('x-token-background');
	if (jsonString) {
...
var SetToken = function (token) {
	if (token) {
		$.cookie('x-token-background', JSON.stringify(token), { expires: token.Expiry.ToDate(), path: '/' });
	} else {
		$.removeCookie('x-token-background', { path: '/' });
	}
};
