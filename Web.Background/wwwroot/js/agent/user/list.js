layui.use(['jquery', 'layer', 'form', 'table', 'laydate', 'list', 'dialog'], function () {
	...
	list.init({
		...
		cols: [[
			...
			{ field: 'AgentName', title: '代理商名称', width: 120, unresize: true, templet: function (d) { return d.Agent.Name; } },
	...
	var active = {
		...
		updateStatus: function (id, status) {
			$.ajax('/agent/user/updateStatus/' + id, {
				type: 'put',
		...
		resetPassword: function (id) {
			layer.confirm('确定重置密码吗？', function (index) {
				$.ajax('/agent/user/password/reset/' + id, {
					type: 'put',
					contentType: 'application/json',
					data: JSON.stringify(''),
					dataType: 'json',
