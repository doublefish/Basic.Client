layui.use(['jquery', 'layer', 'form'], function () {
	...
	if (id > 0) {
		...
		$('#AgentId').val(Token.Data.AgentId);
	...
	$('#Username').on('blur', function () {
		$.ajax('/agent/user/existByUsername/' + id + '/' + this.value, {
			type: 'get',
			async: false,
			dataType: 'json',
			success: function (json) {
	...
	$('#PromoCode').on('blur', function () {
		$.ajax('/agent/user/existByPromoCode/' + id + '/' + this.value, {
			type: 'get',
			async: false,
			dataType: 'json',
			success: function (json) {
