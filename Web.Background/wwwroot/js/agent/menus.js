layui.use(['jquery', 'layer', 'form'], function () {
	...
	var treeObj = $.fn.zTree.init($('#tree'), {
		...
		callback: {
			beforeAsync: function (treeId, treeNode) {
				...
				if (treeNode) {
					treeObj.setting.async.url = '/agent/menu/list/' + treeNode.Id;
				} else {
					treeObj.setting.async.url = '/agent/menu/list/' + Config.Menu.Root;
				}
				return true;
			},
			...
			onRightClick: function (event, treeId, treeNode) {
				...
				$updateStatus.on('click', { id: treeNode.Id, status: status }, function (e) {
					...
					if (status === Config.Status.Disabled && node.isParent) {
						return layer.msg('当前节点有子节点，无法禁用！', { icon: 5, time: 3000, anim: 6 });
					}

					$.ajax('/agent/menu/updateStatus/' + node.Id, {
						type: 'put',
	...
	form.on('submit', function (data) {
		var url = '/agent/menu/add';
		var type = 'post';
		if (data.field.Id > 0) {
			url = '/agent/menu/update/' + data.field.Id;
			type = 'put';
		}
		...
		data.field.Level = data.field.Level.ToInt();
	...
	$('#del').on('click', function () {
		...
		layer.confirm(message, function () {
			$.ajax('/agent/menu/delete/' + node.Id, {
				type: 'delete',
				success: function () {
					layer.msg('已删除');
					//刷新子节点
	...
	//权限控制
	if (HasRight('agent/menu/add')) {
		$('#addChild').show();
		$('#addBrother').show();
		$('#btnSave').show();
		$('#btnReset').show();
	}
	if (HasRight('agent/menu/update')) {
		$('#btnSave').show();
		$('#btnReset').show();
	}
	if (HasRight('agent/menu/updateStatus')) {
		$('#updateStatus').show();
	}
	if (HasRight('agent/menu/delete')) {
		$('#del').show();
	}
	if (HasRight('agent/menu/refreshcache')) {
