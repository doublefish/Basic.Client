layui.use(['jquery', 'layer', 'form'], function () {
	...
	var treeObj = $.fn.zTree.init($('#tree'), {
		...
		data: {
			key: { name: 'Name' },
			simpleData: { enable: true, idKey: 'Id', pIdKey: 'ParentId', rootPId: Config.Region.Root }
		},
		callback: {
			beforeAsync: function (treeId, treeNode) {
				var treeObj = this.getZTreeObj('tree');
				if (treeNode) {
					treeObj.setting.async.url = '/region/list/' + treeNode.Id;
				} else {
					treeObj.setting.async.url = '/region/list/' + Config.Region.Root;
				}
				return true;
			},
			...
			onRightClick: function (event, treeId, treeNode) {
				...
				$updateStatus.on('click', { id: treeNode.Id, status: status }, function (e) {
					...
					$.ajax('/region/updateStatus/' + node.Id, {
						type: 'put',
	...
	form.on('submit', function (data) {
		...
		var type = 'post';
		if (data.field.Id > 0) {
			url = '/region/update/' + data.field.Id;
			type = 'put';
		}
	...
	//权限控制
	if (HasRight('region/add')) {
		$('#addChild').show();
		$('#addBrother').show();
		$('#btnSave').show();
		$('#btnReset').show();
	}
	if (HasRight('region/update')) {
		$('#btnSave').show();
		$('#btnReset').show();
	}
	if (HasRight('region/updateStatus')) {
		$('#updateStatus').show();
	}
	if (HasRight('region/delete')) {
		$('#del').show();
	}
	if (HasRight('region/refreshcache')) {
