layui.use(['jquery', 'layer', 'form'], function () {
	...
	var treeObj = $.fn.zTree.init($('#tree'), {
		...
		data: {
			key: { name: 'Name' },
			simpleData: { enable: true, idKey: 'Id', pIdKey: 'ParentId', rootPId: Config.Dict.Root }
		},
		callback: {
			beforeAsync: function (treeId, treeNode) {
				var treeObj = this.getZTreeObj('tree');
				if (treeNode) {
					treeObj.setting.async.url = '/dict/list/' + treeNode.Id;
				} else {
					treeObj.setting.async.url = '/dict/list/' + Config.Dict.Root;
				}
				return true;
			},
			...
			onRightClick: function (event, treeId, treeNode) {
				...
				$updateStatus.on('click', { id: treeNode.Id, status: status }, function (e) {
					...
					$.ajax('/dict/updateStatus/' + node.Id, {
						type: 'put',
