layui.use(['jquery', 'layer', 'laytpl', 'element', 'table', 'carousel', 'setter', 'view', 'admin', 'echartsTheme'], function () {
	var $ = layui.jquery,
		layer = layui.layer,
		laytpl = layui.laytpl,
	...
	var chartOptions = [{
		title: {
			text: '今日流量趋势', x: 'center', textStyle: { fontSize: 14 }
		},
		...
		yAxis: [{ type: 'value' }],
		series: [{
			name: 'PV', type: 'line', smooth: !0, itemStyle: { normal: { areaStyle: { type: 'default' } } },
			data: [111, 222, 333, 444, 555, 666, 3333, 33333, 55555, 66666, 33333, 3333, 6666, 11888, 26666, 38888, 56666, 42222, 39999, 28888, 17777, 9666, 6555, 5555, 3333, 2222, 3111, 6999, 5888, 2777, 1666, 999, 888, 777]
		},
		{
			name: 'UV', type: 'line', smooth: !0, itemStyle: { normal: { areaStyle: { type: 'default' } } },
	...
	var $charts = $('#LAY-index-dataview').children('div');
	var chartChange = function (i) {
		charts[i] = echarts.init($charts[i], echartsTheme);
		charts[i].setOption(chartOptions[i]);
	...
	if ($charts[0]) {
		...
		carousel.on('change(LAY-index-dataview)', function (e) {
			chartChange(i = e.index);
		});
		admin.on('side', function () {
			setTimeout(function () { chartChange(i); }, 300);
		});
		admin.on('hash(tab)', function () {
			layui.router().path.join('') || chartChange(i);
		});
	}
	...
	table.render({
		...
		url: window.location.origin + '/json/console/top-search.js',
		page: !0,
		cols: [[
			...
			{ field: 'userNums', title: '用户数', sort: !0 }]],
		skin: 'line'
	});
	table.render({
		elem: '#LAY-index-topCard',
		//url: layui.setter.base + 'json/console/top-card.js',
		url: window.location.origin + '/json/console/top-card.js',
		page: !0,
		cellMinWidth: 120,
		cols: [[
			{ type: 'numbers', fixed: 'left' },
